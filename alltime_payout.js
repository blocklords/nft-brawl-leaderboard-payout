/**
 * @description AllTimePayout.js sets the Prizes for all time leaderboard winners on smartcontract.
 ...
 * ---------------------------------------------------------------------------------
 * 7. Connect to API and fetch the leaderboard data for the day after since last paid out day.
 * 
 * 
 *---------------------------------------------------------------------------------
 * Payout exec.
...
let payout = async function () {
	...
	if (announcement.minted) {
		throw `Alltime winners were announced before`;
	}

	let {endTime} = date.sessionDates(session);
	
	let currentTime 	= new Date();
	// Current time in blockchain should be greater than pay time
	if (currentTime.getTime() < endTime.getTime()) {
		throw `Alltime winners could be announced after session end. Current Time:  ${currentTime.toUTCString()}, need to invoke tx at: ${endTime.toUTCString()}`;
	}

	let payUrl = seascapeApi.allTimeUrl(sessionId);

	let data = await seascapeApi.allTimeWinners(payUrl); 
	...
	let txid = await leaderboard.announceAllTimeWinners(nftBrawl, data, gasPrice, admin);
	return {txid: txid, date: new Date()};
};
